...
Decoder::Decoder(QWidget *parent): QWidget(parent) {
    ...
    /** Definindo opções de decodificação e validando itens */
    ...
    foreach (QtAV::VideoDecoderId vid, vds_all) if (!vids.contains(vid)) all.push_back(vid);
...
void Decoder::videoDecoderEnableChanged() {
    QStringList names;
    foreach (DecoderItemWidget *iw, decItems) if (iw->isChecked()) names.append(iw->name());
...
QVariantHash Decoder::videoDecoderOptions() const {
    ...
    foreach (DecoderItemWidget* diw, decItems) options[diw->name()] = diw->getOptions();
    return options;
}
