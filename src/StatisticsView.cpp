...
StatisticsView::StatisticsView(QWidget *parent) : QDialog(parent) {
    ...
    view1 = new QTreeWidget();
    view1->setHeaderHidden(true);
    view1->setColumnCount(2);
    view1->addTopLevelItems(baseItems);
    CTIME = baseItems[5];

    view2 = new QTreeWidget();
    view2->setHeaderHidden(true);
    view2->setColumnCount(2);
    view2->addTopLevelItems(videoItems);
    FPS = videoItems[6];

    view3 = new QTreeWidget();
    view3->setHeaderHidden(true);
    view3->setColumnCount(2);
    view3->addTopLevelItems(audioItems);
    ...
    view1->header()->setSectionResizeMode(QHeaderView::ResizeToContents);
    view2->header()->setSectionResizeMode(QHeaderView::ResizeToContents);
    view3->header()->setSectionResizeMode(QHeaderView::ResizeToContents);
...
void StatisticsView::setStatistics(const Statistics& s) {
    ...
    visibility();
    int csize = view1->header()->sectionSize(0) + view1->header()->sectionSize(1);
    if (csize < view2->header()->sectionSize(0) + view2->header()->sectionSize(1))
        csize = view2->header()->sectionSize(0) + view2->header()->sectionSize(1);
    if (csize < view3->header()->sectionSize(0) + view3->header()->sectionSize(1))
        csize = view3->header()->sectionSize(0) + view3->header()->sectionSize(1);
    csize = csize + 40;
    qDebug("%s(%sDEBUG%s):%s Ajustando comprimento da janela em %i ...\033[0m", GRE, RED, GRE, BLU, csize);
